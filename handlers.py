...
async def vod_name3(msg: Message, state: FSMContext):
    ...
    await msg.answer("", reply_markup=kb.but_cont1)
    Profil.bal = 0
...
@router.callback_query(F.data == "continue1")
async def start_avtorisovaniy(msg: types.CallbackQuery):
    Profil.id_tg = msg.message.from_user.id
    await msg.answer(text.welcome, reply_markup=kb.menu_balance)

    id_1 = msg.chat.id
    print(id_1)
    cursor1.execute('SELECT admin FROM Users WHERE id_tg=?', (int(id_1)))

    cursor1.execute('SELECT * FROM Users WHERE id_tg=?', (int(Profil.id_tg)))
...
async def vod_name3(msg: Message, state: FSMContext):
    ...
    profiles.commit()
        ...
        async def vibor_polzovatelya_admin(msg: types.CallbackQuery):
            cursor1.execute('SELECT * FROM Users WHERE id=?', (str(1)))
            row = cursor1.fetchall()
            for polzovatel in row:
